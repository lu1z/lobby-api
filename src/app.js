...
app.post('/lobbies', async (req, res) => {
    ...
    try {
        ...
        const isDuplicated = await lobbies.findOne({ name });
        ...
        const { insertedId } = await lobbies.insertOne(lobby);
...
app.delete('/lobbies/:name', async (req, res) => {
    ...
    try {
        ...
        const database = client.db(db);
        const lobbies = database.collection('lobbies');
        const { deletedCount } = await lobbies.deleteOne({ name: req.params.name });
...
app.delete('/lobbies', async (req, res) => {
    ...
    try {
        ...
        const database = client.db(db);
        const lobbies = database.collection('lobbies');
        const { deletedCount } = await lobbies.deleteMany({});
        if (!deletedCount) return res.sendStatus(404);
        return res.sendStatus(204);
    } finally {
        if (client) await client.close();
    ...
    try {
        ...
        if (required.every((key) => key in req.body)) {
            if (req.body.op === 'op' && req.body.path === '/status' && (req.body.value === 'open' || req.body.value === 'started')) {
                const { matchedCount, modifiedCount } = await lobbies.updateOne({ name: req.params.name }, { $set: { status: req.body.value } });
